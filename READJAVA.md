...
- 추상화 (Abstraction)  
　추상화는 불필요한 부분을 생략하고 객체의 속성 중 가장 중요한 것에만 중점을 두어 개략화(모델화)하는 것을 의미한다. 이는 최소의 비용으로 실제 상황에 대처할 수 있고, 시스템의 구조 및 구성을 가시적으로 볼 수 있습니다.  


## 21. 동기적 vs 비동기적 프로그래밍

　**동기적(Synchronous)** 프로그래밍은 요청과 그에 따른 결과가 동시에 일어나는 것으로, 특정 작업을 완료한 후에 다음 작업으로 넘어간다. 따라서 서브 루틴 간에 명확한 실행 순서가 존재한다. 이는 설계가 간단하고 직관적이지만 결과가 나올 때까지 대기해야 한다는 단점이 있다.
...
## 22. <a href = "https://m.blog.naver.com/acornedu/221128616501"> Java Servlet vs JSP </a>
 
　**Java Servlet** 은 서버에서 웹페이지 등을 동적으로 생성하거나 데이터 처리를 수행하기 위해 자바로 작성된 프로그램이다. 자바 코드 안에 HTML 태그가 삽입되며 자바 언어로 작성되어 있다. (+ .java 확장자) 이는 자바 언어를 웹 애플리케이션에 조금 더 개발하기 쉽게 하기 위해 만든 API(라이브러리, 클래스 등) 이며 이 규약에 맞는 라이브러리나 클래스를 상속 및 구현하여 만든 클래스를 말한다. (컨트롤러 층)  
 
 　**JSP(Java Server Pages)** 은 HTML 내부에 자바 코드를 삽입하는 형식으로, 서블릿의 단점을 보완하고자 만든 만든 서블릿 기반의 스크립트 기술이다. 서블릿을 이용하여 웹프로그래밍을 할 수 있지만 자바에 대한 지식을 요하며 화면 인터페이스 구현에 너무 많은 코드를 필요로 하는 등 비효율적인 측면이 있다. 이 때문에 서블릿 없이 간편히 웹프로그래밍을 구현하게 만든 기술이 JSP 이다. (<% %> 태그 사용, 프레젠테이션 층)  
